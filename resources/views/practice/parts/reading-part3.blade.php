            ...
            <div class="space-y-4">
                <template x-for="(opt, idx) in currentQuestion.metadata.options" :key="idx">
                    <div class="rounded-lg border border-gray-200 bg-white overflow-hidden shadow-sm">
                        {{-- Colored header with reader name (falls back to A/B/C/D) --}}
                        <div class="px-4 py-2 text-white font-bold text-sm"
                             :class="['bg-blue-500', 'bg-green-500', 'bg-yellow-500', 'bg-purple-500', 'bg-pink-500'][idx] || 'bg-gray-500'">
                            <span x-text="(currentQuestion.metadata.names && currentQuestion.metadata.names[idx]) ? currentQuestion.metadata.names[idx] : String.fromCharCode(65 + idx)"></span>
                        </div>
                        {{-- Text content --}}
                        <div class="p-4">
