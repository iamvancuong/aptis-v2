            ...
            <template x-for="(speaker, sIdx) in currentQuestion.metadata.items" :key="sIdx">
                <div class="border border-gray-200 rounded-lg p-4 bg-white">
                    ...
                    <template x-if="currentQuestion.metadata.audio_files && currentQuestion.metadata.audio_files[sIdx]">
                        <audio :src="(currentQuestion.audio_urls || [])[sIdx]" controls controlslist="nodownload nofullscreen noremoteplayback" oncontextmenu="return false" class="w-full h-10" :id="'speaker_audio_' + sIdx"></audio>
                    </template>
                    <template x-if="!currentQuestion.metadata.audio_files || !currentQuestion.metadata.audio_files[sIdx]">
