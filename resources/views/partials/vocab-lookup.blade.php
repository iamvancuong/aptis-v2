...
@php
    $vocabFolders = auth()->check()
        ? auth()->user()->vocabFolders()->get(['id', 'name'])
...
<div x-data="vocabLookup({
        ...
        folderUrl: '{{ route('vocab.folders.store') }}',
        folders: @js($vocabFolders),
    ...
    <div x-show="panel.show"
        ...
        {{-- Chọn thư mục. Loại từ (danh từ, động từ…) được xếp tự động; thư
             mục ở đây là thư mục học viên tự tạo. --}}
        <div class="px-4 py-2.5 bg-gray-50 border-t border-gray-100 space-y-2"
             x-show="!loading && result">
            <label class="flex items-center gap-2 text-xs text-gray-500">
                <span class="shrink-0">Lưu vào</span>
                <select x-model="folderId" @change="onFolderChange()"
            ...
            <div x-show="newFolder.open" x-cloak class="flex items-center gap-1.5">
                <input x-ref="newFolderInput" x-model="newFolder.name" type="text" maxlength="60"
                       placeholder="Tên thư mục, vd: Chủ đề môi trường"
                       @keydown.enter.prevent="createFolder()" @keydown.escape.stop="cancelNewFolder()"
                       class="flex-1 min-w-0 text-xs px-2 py-1.5 border border-gray-300 rounded-lg focus:outline-none focus:border-indigo-500">
                <button type="button" @click="createFolder()" :disabled="newFolder.busy || !newFolder.name.trim()"
                        class="px-2.5 py-1.5 rounded-lg bg-gray-900 text-white text-xs font-semibold disabled:opacity-50">Tạo</button>
                <button type="button" @click="cancelNewFolder()" class="px-1.5 py-1.5 text-xs text-gray-500 hover:text-gray-800">Huỷ</button>
            </div>
        </div>
