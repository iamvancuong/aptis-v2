...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-900">Edit Set</h1>
</div>

<x-card>
    <form action="{{ route('admin.sets.update', $set) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <select name="quiz_id" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                ...
                @foreach($quizzes as $quiz)
                    <option value="{{ $quiz->id }}" {{ old('quiz_id', $set->quiz_id) == $quiz->id ? 'selected' : '' }}>
                        {{ $quiz->title }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            <label class="flex items-center">
                <input type="checkbox" name="is_public" value="1" {{ old('is_public', $set->is_public) ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600 focus:ring-blue-500">
