...
@section('title', 'Chỉnh sửa Feedback - Admin')
@section('header', 'Chỉnh sửa Feedback')
            ...
            <div class="space-y-6">
                ...
                <div>
                    <div class="flex items-center justify-between mb-2">
                        ...
                        @if($feedback->avatar)
                            <img src="{{ $feedback->avatar }}" alt="Current Avatar" class="w-8 h-8 rounded-full object-cover border border-gray-200">
                        @endif
                    </div>
                    <x-input 
                        type="url"
                        name="avatar" 
                        value="{{ old('avatar', $feedback->avatar) }}" 
                        placeholder="https://example.com/avatar.jpg"
                        error="{{ $errors->first('avatar') }}"
                    />
                    <p class="mt-1.5 text-xs text-gray-500">Bỏ trống hệ thống sẽ tự tạo avatar mặc định dựa trên chữ cái đầu của tên.</p>
                </div>
                ...
                <x-select 
                    ...
                    required
                >
                    <option value="5" {{ old('rating', $feedback->rating) == 5 ? 'selected' : '' }}>5 Sao (★★★★★)</option>
                    <option value="4" {{ old('rating', $feedback->rating) == 4 ? 'selected' : '' }}>4 Sao (★★★★)</option>
                    <option value="3" {{ old('rating', $feedback->rating) == 3 ? 'selected' : '' }}>3 Sao (★★★)</option>
                    <option value="2" {{ old('rating', $feedback->rating) == 2 ? 'selected' : '' }}>2 Sao (★★)</option>
                    <option value="1" {{ old('rating', $feedback->rating) == 1 ? 'selected' : '' }}>1 Sao (★)</option>
                </x-select>
                ...
                <div>
                    ...
                    >{{ old('content', $feedback->content) }}</x-textarea>
                    @error('content')
                ...
                <div class="flex items-center">
                    ...
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $feedback->is_active) ? 'checked' : '' }}
